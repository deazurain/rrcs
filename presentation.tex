...
\begin{frame}
   	...
   	Gegeven
	\begin{enumerate}
		\item Een set $V$ van activiteiten $\{v_1,\dots,v_n\}$ met een tijdsduur $dur(v_i)$
		\item Een set $E$ precedence constraints met elementen $(v_i, v_j)$ zodanig dat $v_i, v_j \in V$ en $v_i$ voor $v_j$ moet worden uitgevoerd
	\end{enumerate}
	
	Gevraagd
	Een schema waarin voor elke activiteit in $V$ een starttijd is aangewezen zodanig dat aan alle precendence relaties in $E$ voldaan wordt.
\end{frame}
