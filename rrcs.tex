...
The RCS problem schedule solution is denoted by a schedule that consists of a set of the starting times for all the activities $S =  (\start{v_1} \ldots \start{v_n})$ where $v_1 \ldots v_n \in V$.
It is also possible that you don't have a solution because all possible schedules exceed either the maximum project execution time or the available resources.
