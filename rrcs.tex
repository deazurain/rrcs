...
The RCS problem belongs to the class of problems that are \emph{NP-hard}.
According to the \TODO{the?} complexity theory a problem belongs to the class \emph{NP-hard} if its decision \TODO{schrijf je dit woord zo? (MickL: ja)} version is \emph{NP-complete} \TODO{Een probleem zit in NPC als hij NP Hard is én in NP zit, ik vind deze redenatie een beetje achterstevoren}.
...
The time and space complexity of the RCS problem depends on the used algorithms and approaches used to solve the problem.
...
The basic PS problem will sometimes not suffice.
Several extensions to the PS problem have been made of which some will be discussed here. 
...
In depth research has been done on stochastic processing time for project scheduling to model this deviation as has been shown in \cite{brucker99} covering several important problems introduced by using a stochastic distribution for activity processing times.
