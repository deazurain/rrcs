...
\item a set of available resources $R$, split in renewable resources $R^{\rho}$ and  consumable (nonrenewable) resources $R^V$ but with a possible overlap of doubly constrained resources
\item the availabilities of the resources $Av(R_i)$, so that $Av(R_k)$ denotes the availability of resource $R_k$
\item the per period usage for an activity $v_j$ of resource $R_k$ are denoted by $Usage(v_j, R_k)$, such that it represents the amount of resource $R_k$ used per time period of executing activity $v_j$
\end{itemize}
