...
To understand what this exactly means, this Section will explain what activities, resources and constraints are to the RCPSP. 
...
As we have shown in the above, the RCPSP is a complex problem, and to find an optimal solution exponential time is needed.
...
STPs are represented with a graph, or Simple Temporal Network, which represents the activities and constraint with nodes and edges.
...
Once contention peaks are identified, an algorithm is able to select the most critical contention peak (for example where the resource capacity excedence is the highest). This contention peak contains a set of activities of which the combined resource usage exceeds the capacity of the resource. In order to reduce the load on a resource, two activities in this set need to be scheduled after each other instead of in parallel. Two activities $(v_i,v_j)$ that belong to the same contention peak are called a \emph{conflict}. 
