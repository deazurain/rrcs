...
In the running example we have six activities. Figure \ref{fig:resource_graph} shows a complete assignment of indexes to the activities. 
...
In the running example the activities have the following duration: $\dur{1} = 3, \dur{2} = 2, \dur{3} = 2, \dur{4} = 1, \dur{5} = 5, \dur{6} = 1$ as can also be seen in Figure \ref{fig:resource_graph}. 
...
The resources of a project are specified by a set $R = \{r_1, \ldots, r_m\}$.
The total amount of resources is $m = |R|$ and each element $r_i \in R$ represents the i-th resource. 
In the running example we have two resources: the sanding machine $r_1$ and the painting brushes $r_2$.
...
\begin{figure}[h]
	...
	\caption{A schedule for the running example. }
	\label{fig:time_feasible_schedule}
\end{figure}

A \emph{schedule} is specified by a vector $S = (\start{1}, \ldots, \start{n})$.
In this schedule each activity $v_i \in V$ has a starting time $\start{i}$ which is defined in the schedule $S$.
A possible schedule is shown in Figure \ref{fig:time_feasible_schedule}. This schedule is notated as $(0, 0, 3, 2, 0, 6)$. 
...
The makespan of the schedule in Figure \ref{fig:time_feasible_schedule} happens to be optimal. 
There exists no assignment of start times so that the end time of the last activity is sooner than $t_6$. 
...
A \emph{feasible schedule} is denoted as $S_f$ and takes the precedence, resource and deadline constraints into account. 
Showing that the schedule from Figure \ref{fig:time_feasible_schedule}, hereafter referred to as schedule A, is completed before the deadline $t_10$ is simple. 
The resource feasibility however is a little harder to demonstrate. 
Figure \ref{fig:feasible_schedule} gives an overview of schedule A and its resource usage over time. 
The capacity of the resources is marked by the dotted line. The total usage of a resource at each period is denoted by the height of the activities in the graph.
The schedule is feasible with respect to the resources because there is no resource of which the capacity is exceeded. 
...
\label{text:PCP}
As seen in the previous section, solving an STP provides us with a set time-feasible schedules. 
...
In order to make the time-feasible solution also comply with the resource capacity, a method called \emph{Precedence Constraint Posting} (PCP) \cite{policella07} is used. 
...
\begin{figure}[h]
	...
	\caption{A time-feasible schedule for the running example along with the the resource profiles.}
	\label{fig:resource_graph}
\end{figure}
