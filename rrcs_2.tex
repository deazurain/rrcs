...
Every day, there are certain tasks we have to complete. 
Finding a suitable schedule to do all of these activities may sometimes be trivial. 
In other cases however, this may not be the case. 
If you have ever found yourself working on a project you will know that it is not always easy to plan all the activities, especially when there are for example multiple people involved. 
...
The trains that are utilized by a Dutch railway company need to undergo regular maintenance. 
This happens in special service bays who might be specialised in maintaining specific parts of a train. 
There are for example service bays that specialize in treating the electrical wiring or in maintaining the wheels.
In scheduling the maintenance of these trains, there are several constraints that need to be satisfied by a schedule before it can be considered to be `good'. 
It is for example desirable to minimize the time that a train is in maintenance to maximize the use of the trains and the service bays as well. 
There is a limited amount of service bays and some jobs can only be performed in specific service bays. 
...
This model is flexible enough to give for example an accurate enough representation of for example the train and airport scheduling problems. 
What we mean by accurate enough is that a solution to the model can be used as a schedule in practice. 
Specific extensions to the RCPSP can be made to better accomodate for a specific problem but a detailed description of known extensions is beyond the scope of this paper.

% Schedules vinden is moeilijk
Practical applications of scheduling show us that there are additional requirements that a schedule needs to fulfill. 
Finding an optimal schedule is proved to be undoable in polynomial time. 
We will discuss an approximation method that attempts to find a schedule that usable in practice. 
The particular technique is called \emph{Precedence Contraint Posting} (PCP) and can be run in polynomial time. 

\begin{comment} Deze tekst weergeeft eigenlijk een beetje de structuur van de paper . Het is misschien wel handig om 
...
A detailed explanation of what the RCPSP precisely entails is given in chapter \ref{text:RCPSP}. 
...
% Purpose of this paper and intended audience
The purpose of this paper is to inform the reader on the topic of resource constrained project scheduling in continuous applications.
\TODO{continuous NA-AH}
...
Although the reader should be familiar with some scientific notation and theories to understand everything completely, the basic ideas described in this paper aim to be complete and clearly explained for everyone. 

\newpage

\section{Problem background \TODO{Misschien een andere titel omdat we STN's niet meer bespreken en alleen RCPSPen de running example uit de doeken doen}}
...
\TODO{wat is het verhaal hier achter? Gaat het niet te diep in op RCPSP? Ik moet denk ik even pauze nemen}
{\itshape
...
\subsection{RCPSP in general \TODO{meh title}}
The RCPSP concerns itself with finding a suitable schedule for a set of activities. 
What the components of a RCPSP instantation are and what suitable actually implies will be explained in this section. 

Every scheduling problem has a set of activities of some sort and this is no different for the RCPSP. 
These activities may also be referred to as jobs or tasks but we will adhere to describing them as activities. 
Part of a RCPSP instantation specifies every activity as well as a duration for every activity in the project. 
This is usually an estimate of the actual time required to complete the activity because the exact duration is not known beforehand most of the time. 
...
An experienced painter\TODO{is dit hoe je een iemand noemt die schildert?} can provide good estimates to the project manager of how long it will take to sand the fence and how much time he will have to spend painting it. 

A RCPSP instantation can also specify certain constraints. 
...
\begin{figure}[h]
	...
	\caption{Two possible schedules for the running example. }
