...
And the following precedence constraints $E$: sanding the door before painting the door $(v_1, v_3)$, sanding the fence before painting the fence $(v_2, v_4)$ and all the painting activities need to be done before cleaning the painting brushes $\{(v_3, v_6), (v_4, v_6), (v_5, v_6)\}$.
...
Since this is not the first painting job the workers have done, they know the exactly what the duration is for each activity:
\begin{itemize}
...
So now they only have two painting brushes $r_1$ and one sanding machine $r_2$ (\emph{resource constraints}), which makes it impossible to sand more than one surface at the same time and to paint more than two surfaces at the same time.
...
Since every sanding job uses the sanding machine as a resource $r_2$, the usage of sanding the door $v_1$ and sanding the fence $v_2$ is $1$ for the sanding machine $r_2$ ($\usage{1}{2} = 1, \usage{2}{2} = 1$).
Also every painting job uses the painting brush as a resource $r_1$, so that painting the door $v_3$, painting the fence $v_4$ and painting the wall $v_5$ all have a usage of $1$ on the painting brush $r_1$ ($\usage{3}{1} = 1, \usage{4}{2} = 1, \usage{5}{1} = 1$).
There is only one activity left, cleaning the painting brushes $v_6$, which will use both of the painting brushes resource $r_1$ so that the usage on the painting brushes $r_1$ is $2$ ($\usage{6}{1} = 2$).
